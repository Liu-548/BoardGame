...
export type PendingAction =
  ...
  // docs/bang-rules/Luat_Bang_Mo_Rong_HighNoon.txt, ví dụ lá "A Fistful of Cards"). MỌI nguồn
  ...
  // `skippedIds` (không có/rỗng ở mọi ván không có The Nobody) = playerId của
  // những người đã bị "bỏ qua hoàn toàn" (draw! ra Bích TRƯỚC khi lá lật —
  // xem GameState.pendingGeneralStore/playGeneralStore()/resolveDrawCheck()
  // nhánh "the_nobody_store") — KHÔNG được phát bài, phải loại khỏi vòng chọn
  // (respondToStorePick() dùng field này để bỏ qua khi tính người chọn kế tiếp).
  | { kind: "NEED_PICK_STORE_CARD"; player: string; options: string[]; skippedIds?: string[] }
  ...
  // đẩy SAU KHI đã rút 2 lá đầu lượt bình thường VÀ draw! (NEED_DRAW_CHECK,
  // source.card = "the_thief") vừa khớp chất đỏ (Cơ/Rô) — xem completeDrawPhase()/
  // resolveDrawCheck() trong reduce.ts. RESPOND kèm targetId = cướp NGẪU
  // NHIÊN 1 lá của người đó (tay rỗng thì không lấy được gì, KHÔNG rút bù);
  // không kèm targetId (mặc định) = bỏ qua, không lấy gì cả — CHỈ đúng nghĩa
  // này khi CHÍNH người chơi tự bỏ qua, KHÔNG PHẢI hành vi mặc định lúc hết
  // giờ (room.ts hết giờ phải tự CHỌN NGẪU NHIÊN 1 người còn sống CÓ BÀI, chỉ
  // rơi về "không chọn ai" nếu thật sự không còn ai có bài — xem
  // buildReactiveTimeoutAction()).
  | { kind: "NEED_PICK_THIEF_TARGET"; player: string }
  ...
  // 3 CLAUDE.md), xem respondToUseDrifterShield() trong reduce.ts.
  //
  // QUAN TRỌNG (đã tự rà lại và chốt kiến trúc): pending này được đẩy VÔ
  // ĐIỀU KIỆN mỗi khi mục tiêu có hasDrifterShield, KỂ CẢ KHI
  // drifterShield[player] hiện đang false — nếu chỉ đẩy lúc có khiên thật thì
  // chính SỰ XUẤT HIỆN của pending đã lộ bí mật cho mọi người xem
  // (state.pending công khai, viewPendingItem() không lọc kind này). Giá trị
  // thật chỉ được đọc lúc GIẢI QUYẾT, y hệt cách NEED_MISSED vẫn hỏi dù người
  // chơi không có lá Missed! nào trong tay.
  //
  // RESPOND kèm `useShield: true` = muốn dùng (chỉ THỰC SỰ có tác dụng nếu
  // drifterShield[player] đang true — nếu không, coi như từ chối, KHÔNG báo
  ...
  // respondToUseDealerTrade() biết tiếp tục đúng luồng gốc (Mary Rose reflect,
  // continueAfterMissedResolved()...).
  //
  // RESPOND kèm `useDealerTrade: true` = đưa 2 lá ngẫu nhiên cho người vừa
  ...
  // hồi sinh `targetId` với ĐÚNG 1 máu (chỉ thật sự có tác dụng nếu
  // sentinelUsed[player] hiện đang false — nếu không, coi như từ chối, không
  // báo lỗi khác biệt); không kèm/false = từ chối (KHÔNG tiêu hao lượt dùng —
  // chỉ đồng ý mới đánh dấu sentinelUsed, giống nguyên tắc drifterShield chỉ
  // mất khi THẬT SỰ dùng).
  | {
...
// turn-start/Dynamite/Russian Roulette).
export type DamageResume =
  | { kind: "indians" }
  | { kind: "bang_missed"; missedTop: PendingAction & { kind: "NEED_MISSED" } }
  | { kind: "duel"; opponent: string }
  | { kind: "high_noon_turn_start" }
  | { kind: "dynamite" }
  | { kind: "russian_roulette" };

// Bộ mở rộng "custom_characters" (The Nobody, xem docs/bang-rules/House_Rule.txt mục I, NHÓM
// B) — dữ liệu "còn phải làm gì tiếp" cho 4/5 lá cần hoãn (Tequila không cần,
// xem GameState.pendingNobodyCheck). `stolenCardId`/`stolenFromHand` (Panic!)
// đã XÁC ĐỊNH SẴN lúc đánh bài (giữ tính tất định của RNG — không phụ thuộc
// draw! của The Nobody), chỉ hoãn phần THẬT SỰ chuyển lá. `cardId` (Jail) =
// chính lá Jail đang chờ gắn lên sân hoặc bỏ vào chồng bỏ. `remainingCheckIds`
// (Saloon) = những người khác còn phải hỏi tiếp (hiếm — Vera Custer mượn
// cùng lúc The Nobody thật), đúng mẫu remainingCheckIds của
// GameState.pendingGeneralStore. `marcelId` (Marcel companion) = ai đang chờ
// biết "cùng vào tù" với mình.
export type PendingNobodyCheck =
  | { kind: "panic"; playerId: string; stolenCardId: string; stolenFromHand: boolean }
  | { kind: "jail"; cardId: string }
  | { kind: "saloon"; remainingCheckIds: string[] }
  | { kind: "marcel_companion"; marcelId: string };
...
export type GameEvent =
  ...
  // Bộ mở rộng "custom_characters" (The Thief, xem docs/bang-rules/House_Rule.txt mục I) —
  // draw! cộng thêm sau pha rút khớp chất đỏ, cướp NGẪU NHIÊN `cardId` của
  ...
  // Bộ mở rộng "custom_characters" (The DareDevil, xem docs/bang-rules/House_Rule.txt mục
  ...
  // "bị tấn công"). Mất máu này NGAY LẬP TỨC, không hoàn lại dù mục tiêu đỡ
  // được — xem useFairKillerShot() trong reduce.ts.
  | { type: "FAIR_KILLER_TRADED_LIFE"; playerId: string; targetId: string }
  ...
  // (PLAYER_ELIMINATED vẫn bắn như mọi ca chết khác).
  | { type: "SENTINEL_REVIVED"; playerId: string; sentinelId: string; sentinelNewMaxHp: number }
  // Bộ mở rộng "custom_characters" (The Nobody, xem docs/bang-rules/House_Rule.txt mục I) —
...
export interface GameState {
  ...
  // docs/bang-rules/NHAN-VAT-BANG-CO-BAN.txt): luật gốc chỉ cho đánh 1 lá Bang!/lượt, trừ khi
  ...
  // Mở rộng Dodge City (docs/bang-rules/Luat_Bang_Mo_Rong_DodgeCity.txt, mục 1.1 — "kiến trúc
  // trang bị trì hoãn", xem docs/roadmap/LO-TRINH.md "Ghi chú cho 5.4" mục A) — đếm số
  ...
  // playerId) vì assertCurrentPlayer() đã đảm bảo chỉ CHÍNH người đang tới
  ...
  // Bộ mở rộng "custom_characters" (The Nobody, xem docs/bang-rules/House_Rule.txt mục I) —
  // Cửa hàng tổng hợp cần BIẾT TRƯỚC có ai bị "bỏ qua" (draw! ra Bích) hay
  // không TRƯỚC KHI lật bài (số lá lật = số người sống - số người bị bỏ qua),
  // nên playGeneralStore() không lật ngay như bình thường — đẩy CHUỖI
  // NEED_DRAW_CHECK (source.card = "the_nobody_store") cho TỪNG người có khả
  // năng này (thường chỉ 1, hiếm khi 2 — The Nobody thật VÀ Vera Custer đang
  // mượn), field này giữ phần "còn phải làm gì tiếp" giữa các lần draw! đó.
  // null = không có Cửa hàng tổng hợp nào đang ở giai đoạn kiểm tra The Nobody
  // (dùng bình thường, một-giai-đoạn, ở MỌI ván không có The Nobody).
  pendingGeneralStore: { cardId: string; initiatorId: string; remainingCheckIds: string[]; skippedIds: string[] } | null;
  // Bộ mở rộng "custom_characters" (The Nobody, xem docs/bang-rules/House_Rule.txt mục I,
  // NHÓM B) — 5 lá "áp dụng NGAY LẬP TỨC, không có pending sẵn để huỷ" (khác
  // hẳn nhóm A, chỉ cần đẩy NEED_DRAW_CHECK lên trên 1 pending có sẵn) — phải
  // HOÃN LẠI phần áp dụng, giữ "còn phải làm gì tiếp" ở đây trong lúc chờ
  // draw! của The Nobody giải quyết xong (xem resolveDrawCheck() các nhánh
  // "the_nobody_panic"/"the_nobody_jail"/"the_nobody_saloon"/
  // "the_nobody_marcel_companion" — Tequila KHÔNG cần field này, chỉ cần
  // top.player là đủ, xem playTequila()). GỘP CHUNG 1 field discriminated
  // union (thay vì 5 field riêng) — tránh phình GameState với nhiều field
  ...
  // false. Chỉ bị GHI ĐÈ mỗi khi draw! bí mật đầu lượt CỦA CHÍNH người đó chạy
  // xong (xem completeDrawPhase()/resolveDrawCheck() trong reduce.ts) —
  // KHÔNG reset vô điều kiện ở advanceTurn() như bangCountThisTurn, đúng
  // "hết hạn vào ĐẦU LƯỢT KẾ TIẾP của CHÍNH anh ta" (không phải "mỗi lượt của
  // bất kỳ ai" — field tự nhiên "sống sót" qua lượt người khác xen giữa vì
  // không có chỗ nào khác ghi đè nó).
  //
  // BÍ MẬT — KHÁC elenaNoirArmed (field đó công khai vì không cần giấu gì).
  // viewFor() (view.ts) CHỈ được trả entry của ĐÚNG viewerId, xoá hẳn mọi
  ...
  // mất khỏi deck/discardPile/tay/sân), đẩy vào discardPile ở thời điểm AN
  // TOÀN (đã hết hạn, không còn ý nghĩa chiến thuật gì — xem
  // resolveDrawCheck()/eliminatePlayer() trong reduce.ts). Không có entry =
  // không có lá nào đang "treo" bí mật.
  drifterHiddenCard: Record<string, string>;
  // Bộ mở rộng "custom_characters" (Marcel Marcelo, xem docs/bang-rules/House_Rule.txt mục I)
  ...
  // reduce.ts) phải rút 3 lá thay vì 2 (Phương án C, xem docs/bang-rules/House_Rule.txt mục
  ...
  // false — không bao giờ xoá key (khác các Record "tạm thời" khác ở trên,
  // đây là dấu vết VĨNH VIỄN của cả ván).
  sentinelUsed: Record<string, boolean>;
  // Mở rộng High Noon/A Fistful of Cards (xem core/events.ts + mục 1.2
  // docs/bang-rules/Luat_Bang_Mo_Rong_HighNoon.txt) — chồng lá sự kiện chưa lật. Kiểu
...
// Giai đoạn 5, việc 5.3 — danh sách id luật bổ sung đã cài (xem docs/roadmap/LO-TRINH.md
...
export type HouseRuleId =
  ...
  // Gấp đôi MỌI mốc thời gian đếm ngược (lượt đánh 60s/phản hồi 15s/bỏ bài
  // thừa 15s/chọn nhân vật 30s) — NGOẠI LỆ DUY NHẤT trong HouseRuleId: xử lý
  // ở server/room.ts (scheduleDeadline()), KHÔNG PHẢI reduce.ts/distance.ts
  // như mọi luật khác, vì đồng hồ đếm ngược vốn CHỈ tồn tại lúc chơi qua
  // mạng (core/ không được biết gì về thời gian/mạng, quy tắc 1+2 CLAUDE.md)
  // — hotseat KHÔNG có đồng hồ nào để tăng, luật này không có tác dụng gì
  // khi chơi hotseat.
  | "double_timers";
...
// docs/bang-rules/House_Rule.txt), tên hiển thị luôn có đuôi "*ex". Không thêm lá bài mới
...
// docs/bang-rules/Luat_Bang_Mo_Rong_HighNoon.txt/docs/bang-rules/Luat_Bang_Mo_Rong_FistfulOfCards.txt +
