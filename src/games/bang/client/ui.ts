...
const CARD_LABELS: Record<CardName, string> = {
  ...
  jail: "Nhà tù",
  dynamite: "Thuốc nổ",
  // Mở rộng Dodge City (đợt 1/40 lá — xem docs/bang-rules/Luat_Bang_Mo_Rong_DodgeCity.txt).
  bible: "Kinh Thánh",
...
const CARD_DESCRIPTIONS: Record<CardName, string> = {
  ...
  // trong reduce.ts (xem docs/bang-rules/Luat_Bang_Mo_Rong_DodgeCity.txt).
...
// đầu docs/bang-rules/Luat_Bang_Mo_Rong_DodgeCity.txt: xanh lá ĐÃ dành riêng cho khung nhân
...
// public/sprites/<tên lá>.png sau (đúng tinh thần docs/roadmap/LO-TRINH.md: "có ảnh tới đâu
...
// file đó trước khi viết, không chép nguyên văn docs/bang-rules/NHAN-VAT-BANG-CO-BAN.txt vì
...
const CHARACTER_DESCRIPTIONS: Record<string, string> = {
  ...
  // Bộ mở rộng "custom_characters" (xem docs/bang-rules/House_Rule.txt mục I) — nhân vật TỰ
...
//
// Bổ sung theo yêu cầu chủ dự án (docs/roadmap/LO-TRINH.md, 2026-08-05) — hiện thêm LƯỢNG
...
const EXPANSION_DESCRIPTIONS: Record<ExpansionId, string> = {
  ...
  // Nhân vật TỰ CHẾ (không thuộc bản gốc/Dodge City, xem docs/bang-rules/House_Rule.txt) —
...
// Mặc định TỐI khi chưa từng chọn gì (localStorage trống) — chủ dự án yêu
// cầu đổi mặc định (trước đây mặc định sáng). Ai đã bấm "Sáng" trong Cài đặt
// (lưu đúng chữ "light") vẫn giữ nguyên lựa chọn đó, không bị ép về tối.
...
// Bản Beta song song (docs/roadmap/LO-TRINH.md) — chỉ mở URL khác ở TAB MỚI, không có
...
// riêng của dự án, 4-8 người theo luật gốc BANG!, xem docs/roadmap/LO-TRINH.md). Đây là
...
const MIN_PLAYERS = 2; // biến thể 2 người (xem docs/roadmap/LO-TRINH.md) — setup.ts's isDuelMode()
const MAX_PLAYERS = 8; // biến thể 8 người (xem docs/roadmap/LO-TRINH.md) — setup.ts's ROLE_SETS đã hỗ trợ
...
// Bản Beta song song (docs/roadmap/LO-TRINH.md) — label + href tính sẵn ở main.ts (đọc
...
// Sảnh chọn game (src/hub/) — nút quay lại dùng chung được ở bất cứ đâu cần,
// hiện chỉ dùng ở đây (màn "Chọn cách chơi" là màn DUY NHẤT đứng ngay sau
// Sảnh, mọi màn con khác của Bang! quay ngược lại đây trước, không quay
// thẳng ra Sảnh).
...
// Nút "Chơi cùng bạn bè" cần thêm 1 dòng chú thích ngắn dưới nhãn chính —
// button() thường (1 dòng textContent) không đủ, nên tách riêng ở đây.
// `featured` tô đậm màu đỏ cho lựa chọn được ưu tiên (xem .mode-btn--featured,
// style.css) — quyết định lúc duyệt bản xem trước giao diện (Artifact).
function modeButton(label: string, hint: string, onClick: () => void, featured: boolean): HTMLButtonElement {
  const el = document.createElement("button");
  el.type = "button";
  el.className = "mode-btn" + (featured ? " mode-btn--featured" : "");
  el.addEventListener("click", onClick);
  const labelEl = document.createElement("span");
  labelEl.textContent = label;
  const hintEl = document.createElement("span");
  hintEl.className = "mode-btn__hint";
...
export function renderHomeScreen(container: HTMLElement, betaLink: BetaLinkInfo, handlers: HomeHandlers): void {
  ...
  container.appendChild(backLink("← Quay lại Sảnh", () => handlers.onBackToHub()));

  // "Vé" kiểu miền Tây thay cho tiêu đề chữ suông cũ — xem .ticket*, style.css
  // (giống hệt bản xem trước đã duyệt).
  const ticket = document.createElement("div");
  ticket.className = "ticket";

  const ticketHead = document.createElement("div");
  ticketHead.className = "ticket__head";
  const wordmark = document.createElement("p");
  wordmark.className = "ticket__wordmark";
  wordmark.textContent = "Bang!";
  const tag = document.createElement("p");
  tag.className = "ticket__tag";
  tag.textContent = "CHỌN CÁCH CHƠI";
  ticketHead.appendChild(wordmark);
  ticketHead.appendChild(tag);
  ticket.appendChild(ticketHead);

  const ticketBody = document.createElement("div");
  ticketBody.className = "ticket__body";
  ticketBody.appendChild(
    modeButton("Chơi cùng bạn bè", "Tạo phòng, gửi mã — mỗi người một máy, ở đâu cũng chơi được", () => handlers.onPlayNetwork(), true)
  );
  ticketBody.appendChild(
    modeButton("Chơi chung một máy", "Ngồi cùng nhau, truyền tay 1 điện thoại hoặc máy tính", () => handlers.onPlayLocal(), false)
  );
  ticketBody.appendChild(
    modeButton("Thư viện bài", "Tra cứu lá bài và nhân vật trước khi chơi", () => handlers.onShowCardReference(), false)
  );
  ticketBody.appendChild(linkButton(betaLink.label, betaLink.href));
...
// — đúng số liệu tĩnh của nhân vật, giống cách docs/bang-rules/NHAN-VAT-BANG-CO-BAN.txt ghi).
...
// xem docs/ui-ux/GIAO-DIEN-UI-UX.txt/yêu cầu bổ sung "xem thư viện bài mà không văng ra
