...
**5.4 (Dodge City) — ĐANG LÀM, ĐỦ 40/40 LÁ BÀI (mục A + B + E xong)** (xem changelog "Mở rộng Dodge City — đợt 1" và "đợt 2" trong CHANGELOG.md): kiến trúc trang bị trì hoãn (mục A) + toàn bộ 40 lá bài mới (mục B) + cơ chế "bỏ kèm 1 lá phụ"/Brawl (mục E) — CHỈ core + test, **UI CHƯA có nút bấm thật** (chưa kích hoạt được lá vàng đã bày sẵn, chưa đáp lại NEED_MISSED bằng trang bị, chưa có UI cho lá cần bỏ kèm 1 lá phụ) — **CHƯA deploy, kể cả bản beta**, đợi UI hoặc tới khi cần thử qua `wrangler dev`. House rule "extra_cards" (checkbox đã có từ trước) giờ cộng đủ 40/40 lá vào bộ, mô tả đã cập nhật cảnh báo đúng. Còn lại: 15 nhân vật (mục C, đặc biệt Vera Custer phức tạp nhất — cần cơ chế uỷ quyền toàn hệ thống), luật riêng biến thể 3 người (mục D — thưởng 3 lá khi tự tay hạ bất kỳ ai), UI hiển thị chất bài (mục F), và UI thật cho toàn bộ 40 lá.
