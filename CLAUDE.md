...
**4 nhân vật *ex đợt "dễ trước" (2026-08-10)** — xem `House_Rule.txt` mục I để biết chi tiết luật/thiết kế: **The Thief** (3 máu — draw! cộng thêm sau pha rút bình thường, ra đỏ thì cướp ngẫu nhiên 1 lá của 1 người khác), **The Gambler** (3 máu — USE_ABILITY bỏ 2 lá rồi draw!, đỏ rút 3/đen rút 1, không giới hạn số lần), **The Fair Killer** (4 máu — USE_ABILITY mất 1 máu bắn Bang! bỏ qua khoảng cách, 1 lần/lượt, không hoàn máu dù bị đỡ), **The Drunker** (4 máu — hoàn toàn thụ động, ăn theo +1 máu mỗi khi NGƯỜI KHÁC uống Bia thành công, hook mới `onOtherPlayerHealedByBeer`) đều đã **CODE XONG** (core + UI hotseat/qua mạng, trừ Drunker không cần UI vì thụ động) + test riêng từng nhân vật (`test/the-thief.test.ts`/`the-gambler.test.ts`/`the-fair-killer.test.ts`/`the-drunker.test.ts`, tổng 41 test). Cả 4 đều thuộc bộ mở rộng `"custom_characters"` (checkbox "Nhân vật *ex" lúc thiết lập ván) — cộng 3 nhân vật cũ (Elena Noir/Marcel Marcelo/Mary Rose) thành **7/13 nhân vật *ex đã xong**.

**The Drifter *ex (2026-08-10) — CODE XONG** (core + UI hotseat/qua mạng + test — xem đặc tả đã CHỐT ở `House_Rule.txt` mục I, đoạn "The Drifter 3 máu", KHÔNG phải bản mô tả cũ đã bị bỏ). 3 máu, 2 chất đỏ (Cơ/Rô). Đầu mỗi lượt của chính mình, draw! 1 lá **NGẦM** (chỉ mình biết): đỏ thì có 1 "lá chắn" tới đầu lượt kế tiếp của chính mình; khi sắp mất máu vì bất cứ lý do gì có thể tự chọn dùng lá chắn để chặn **TRỌN** cả cụm sát thương đó (kể cả Thuốc nổ 3 máu). Test: `test/the-drifter.test.ts` (21 test). **2 phát hiện kiến trúc quan trọng lúc code** (không có trong đặc tả gốc, tự rà code phát hiện): (1) "applyDamage() là điểm áp sát thương duy nhất" — SAI, có 6 điểm thật (4 qua `applyDamage()`: Indians!/Bang!-Gatling/Duel/High Noon turn-start; 2 điểm tự trừ máu trực tiếp KHÔNG qua `applyDamage()`: Dynamite/Russian Roulette) — cả 6 đều đã cắm `maybeAskDrifterShield()`; (2) draw! bí mật KHÔNG được tái dùng nguyên xi `NEED_DRAW_CHECK`/`resolveDrawCheck()` thường vì sẽ rò rỉ qua `discardPile` (công khai qua `viewFor()`) lẫn qua `events` (server gửi NGUYÊN VẸN cho MỌI socket, KHÔNG lọc theo viewer, khác `view` — xem `room.ts`'s `sendStateTo()`) — phải đi 1 nhánh riêng trong `resolveDrawCheck()`, giữ lá ở field nội bộ mới `GameState.drifterHiddenCard` (không lộ qua `PlayerView`), không emit event nào. `GameState.drifterShield` (Record theo playerId, để tương thích Vera Custer mượn) cũng là field **BÍ MẬT ĐẦU TIÊN** trong dự án cần `viewFor()` lọc riêng theo viewerId (khác `elenaNoirArmed` công khai) — `PlayerView.drifterShield` chỉ chứa ĐÚNG 1 key của chính viewer nếu có, người khác không thấy key đó dưới bất kỳ dạng nào. `NEED_USE_DRIFTER_SHIELD` (pending mới) đẩy **VÔ ĐIỀU KIỆN** mỗi khi mục tiêu có khả năng này — kể cả đang KHÔNG có khiên thật — vì chỉ đẩy lúc có khiên thật sẽ khiến chính sự XUẤT HIỆN của pending lộ bí mật (đúng mẫu `NEED_MISSED` vẫn hỏi dù không có gì để đỡ). Đã tự kiểm bằng trình duyệt thật (hotseat 8 người, bật "Nhân vật *ex"): tra "The Drifter" trong Thư viện bài ra đúng mô tả, màn hình chọn nhân vật hiển thị đúng, không lỗi console (chưa random trúng đúng The Drifter trong ván thật để tự kiểm luồng dùng khiên qua UI — rủi ro thấp, logic đã có 21 test bao phủ đầy đủ các nhánh).

**The Dealer *ex (2026-08-10) — CODE XONG** (core + UI hotseat/qua mạng + test — xem đặc tả đã CHỐT ở `House_Rule.txt` mục I, đoạn "The Dealer 4 máu"). 4 máu. Mỗi khi bị nhắm bởi 1 đòn "kiểu Bang!" (Bang!, Gatling, Punch, Springfield, Derringer, Knife, Pepperbox, Buffalo Rifle, Howitzer, đòn của Doc Holyday...) và không đỡ được, được chọn đưa **2 lá NGẪU NHIÊN** trên tay cho **NGƯỜI VỪA ĐÁNH mình** để vô hiệu đòn đó — không giới hạn số lần, miễn còn đủ 2 lá. Test: `test/the-dealer.test.ts` (12 test). **Tự rà code trước khi cài và xác nhận CHỈ CÓ ĐÚNG 1 điểm cắm** (khác The Drifter — 6 điểm): mọi lá "kiểu Bang!" đều đi qua CHUNG 1 con đường `pushMissedReactionUnconditional()` → `NEED_MISSED` → `respondToMissed()` — tái dùng gần như nguyên vẹn hạ tầng vừa xây cho Drifter (cùng vị trí chèn trong `respondToMissed()`, ngay trước `applyDamage()`, không xung đột vì 1 người không thể vừa là Drifter vừa là Dealer). **Khác Drifter — KHÔNG cần "hỏi vô điều kiện"**: "còn đủ 2 lá" không phải bí mật (`handCount` vốn đã công khai), nên `NEED_USE_DEALER_TRADE` chỉ đẩy khi THẬT SỰ đủ điều kiện (còn ≥2 lá VÀ có người đánh còn sống để đưa bài — đã tự phát hiện và chặn ca biên: lá sự kiện "A Fistful of Cards" bắn với `source.from: null`, không có ai để đưa bài, phải bỏ qua). Event riêng `DEALER_TRADE_USED` (KHÔNG tái dùng `CARD_STOLEN` có sẵn — đã kiểm `describeEvent()` phrasing "X cướp Y của Z" sai ngữ nghĩa cho hành động tự nguyện). Đã tự kiểm bằng trình duyệt thật: tra "The Dealer" trong Thư viện bài ra đúng mô tả, không lỗi console.

**The Sentinel *ex (2026-08-20) — CODE XONG** (core + UI hotseat/qua mạng + test — xem changelog "The Sentinel *ex" trong CHANGELOG.md). 3 máu (đã hỏi lại chủ dự án để chốt số liệu — đặc tả chính ghi 3 máu, phần đánh giá sức mạnh phía dưới file luật ghi nhầm 4 máu do bản nháp cũ). Bất kỳ lúc nào có người sắp chết (kể cả chính The Sentinel), hỏi SAU CÙNG có muốn trả 2 máu tối đa VĨNH VIỄN để hồi sinh 1 máu hay không — ĐÚNG 1 LẦN CẢ VÁN. Lần đầu dự án sửa `maxHp` giữa ván. Điểm khó nhất: tái dùng kiểu `resume` của The Drifter (đổi tên `DrifterShieldResume` → `DamageResume`, dùng chung) để hoãn đúng phần epilogue phụ thuộc `target.alive` (`continueAfterMissedResolved()`/`continueTurnStartAfterHighNoonDamage()`/`applyJailCheck()`) tới sau khi Sentinel trả lời, còn phần không phụ thuộc alive (Mary Rose reflect/Molly Stark) vẫn chạy ngay, không cần hoãn. Test: `test/the-sentinel.test.ts` (10 test). Đã tự kiểm bằng trình duyệt thật (hotseat 6 người): random trúng đúng nhân vật, hạ máu về 0, hồi sinh chính mình qua UI, máu tối đa/hiện tại/nhật ký đều đúng, không lỗi console.
...
Còn lại The Beggar/Jeff The Clown trong `House_Rule.txt` mục I (Jeff The Clown và The Beggar nên làm SAU CÙNG theo đúng ghi chú trong file luật) — CHƯA làm, để dành đợt sau.
