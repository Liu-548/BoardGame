...
**5.4 (Dodge City) — CORE HOÀN TẤT (mục A-F đều xong, kể cả 15/15 nhân vật mục C), CHỈ CÒN UI** (xem changelog "Mở rộng Dodge City — đợt 1"/"đợt 2"/"đợt 3"/"Vera Custer" trong CHANGELOG.md): kiến trúc trang bị trì hoãn (mục A) + toàn bộ 40 lá bài mới (mục B) + luật riêng biến thể 3 người (mục D — `eliminatePlayer()` thưởng 3 lá khi tự tay hạ BẤT KỲ ai, event `HUNT_KILL_BOUNTY_DRAWN` mới, tách biệt `OUTLAW_BOUNTY_DRAWN`) + cơ chế "bỏ kèm 1 lá phụ"/Brawl (mục E) + UI hiển thị chất bài (mục F — rà lại code phát hiện `cardButton()`/`cardChip()` ĐÃ hiện badge chất/số từ đợt 1, tự kiểm lại bằng trình duyệt thật xác nhận đúng) — CHỈ core + test, **UI CHƯA có nút bấm thật cho lá vàng/lá bỏ kèm** (chưa kích hoạt được lá vàng đã bày sẵn, chưa đáp lại NEED_MISSED bằng trang bị, chưa có UI cho lá cần bỏ kèm 1 lá phụ) — **CHƯA deploy, kể cả bản beta**, đợi UI hoặc tới khi cần thử qua `wrangler dev`. Bộ mở rộng "dodge_city" (bổ sung theo yêu cầu chủ dự án — TÁCH RIÊNG khỏi house rules cũ, xem `ExpansionId` ở `types.ts`) khi bật cộng đủ 40/40 lá vào bộ bài VÀ đưa 15 nhân vật Dodge City vào bộ bốc "phát 2 lá, chọn giữ 1" — kiến trúc đã sẵn cho nhiều bộ mở rộng cùng lúc (mảng `expansions`, mỗi bộ đóng góp `EXPANSION_CARD_COUNTS`/`EXPANSION_CHARACTER_IDS` riêng), dù hiện chỉ có 1 bộ.
